  ...
  <ion-page class="overflow-auto pb-64">
    <div class="z-[40]">
      ...
      <div v-if="!booksLoading">
        <h2 class="text-center mb-4">Read Books</h2>
        ...
        <div v-else class="flex flex-col items-center">
          ...
          <span class="mt-5">You have not marked any books as read yet.</span>
        </div>
      </div>
      <div v-else>
        <v-progress-circular
...
const openQuickActions = (bookId) => {
  selectedBook.value = readBooks.value.find(book => book.id === bookId);
...
onMounted(async () => {
  ...
  navigateTo({ path: '/books/read', query: {} });

  try {
    readBooks.value = bookStore.getReadBooks;
    user.value = authStore.getUser();
  } catch (error) {
    console.error('An error occurred while fetching read books:', error);
  } finally {
