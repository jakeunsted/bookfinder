  ...
  <div class="w-full max-w-md text-center">
    <div class="bg-zinc-900 shadow-lg rounded-lg overflow-hidden">
      ...
      <div class="p-4">
        ...
        <h3 v-if="book.authors && book.authors.length" class="text-lg text-gray-300">
          {{ book.authors.join(', ') }}
        </h3>
        <p class="mt-2">
          {{ truncatedDescription }}
          <span v-if="!showFullDescription" class="text-blue-500 cursor-pointer underline decoration-wavy" @click="toggleDescription">
            Show More
          </span>
          <span v-if="showFullDescription" class="text-blue-500 cursor-pointer underline decoration-wavy" @click="toggleDescription">
            Show Less
        ...
        <p class="mt-2 text-white"><strong>Page Count:</strong> {{ book.pageCount }}</p>
        <p class="mt-2 text-white"><strong>Categories:</strong> {{ book.categories ? book.categories.join(', ') : 'N/A' }}</p>
...
const truncatedDescription = computed(() => {
  if (props.showFullDescription || !props.book) return props.book.description || ''
